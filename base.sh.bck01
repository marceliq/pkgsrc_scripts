...
if [ "$solaris" = true ]; then
  if [ ! -d "${PKGSRC_BASE}/pkgsrc/bootstrap/work" ]; then
    CC=$CC LDFLAGS="-Wl,--strip-all" SHARED_LDFLAGS="-Wl,--strip-all" CFLAGS="-O2 -fstack-protector-strong -fno-common -Wp,-U_FORTIFY_SOURCE,-D_FORTIFY_SOURCE=3 -Wstrict-prototypes" CPPFLAGS="-O2" CXXFLAGS="-O2" ./bootstrap --abi 32 --make-jobs $PJOBS --prefer-pkgsrc no --prefix $PREFIX --unprivileged --sysconfdir $CONFDIR || exit 1
  fi
else
  if [ ! -d "${PKGSRC_BASE}/pkgsrc/bootstrap/work" ]; then
    LDFLAGS="-Wl,--strip-all" SHARED_LDFLAGS="-Wl,--strip-all" CFLAGS="-O2 -fstack-protector-strong -Wp,-U_FORTIFY_SOURCE,-D_FORTIFY_SOURCE=2" CPPFLAGS="-O2 -fstack-protector-strong -Wp,-U_FORTIFY_SOURCE,-D_FORTIFY_SOURCE=2" CXXFLAGS="-O2 -fstack-protector-strong -Wp,-U_FORTIFY_SOURCE,-D_FORTIFY_SOURCE=2" ./bootstrap --abi 64 --make-jobs $PJOBS --prefer-pkgsrc yes --prefix $PREFIX --unprivileged --sysconfdir $CONFDIR --varbase $VARBASE || exit 1
...
#  props="CXXFLAGS-=\t\t-O2 MAKE_JOBS=\t\t$PJOBS SKIP_LICENSE_CHECK=\tyes"
