...
VERSION=`$PREFIX/sbin/pkg_info | ${GREP} nmap | ${AWK} -F '-' '{print $2}' | ${AWK} '{print $1}'`
...
# vytvoreni balicku
(cd $PREFIX-${VERSION}/.. && tar czf nmap-${VERSION}-`uname -s | tr '[:upper:]' '[:lower:]'`-`uname -p`.tar.gz nmap-${VERSION}) || exit 1
