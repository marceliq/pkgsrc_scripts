...
if [ "$solaris" = true ]; then
  if [ ! -d "${PKGSRC_BASE}/pkgsrc/bootstrap/work" ]; then
    CC=$CC LDFLAGS="-Wl,--strip-all" SHARED_LDFLAGS="-Wl,--strip-all" CFLAGS="-O3" CPPFLAGS="-O3" CXXFLAGS="-O3" ./bootstrap --abi 32 --make-jobs $PJOBS --prefer-pkgsrc no --prefix $PREFIX --unprivileged --sysconfdir $CONFDIR || exit 1
  fi
else
  if [ ! -d "${PKGSRC_BASE}/pkgsrc/bootstrap/work" ]; then
    LDFLAGS="-Wl,--strip-all" SHARED_LDFLAGS="-Wl,--strip-all" CFLAGS="-O3" CPPFLAGS="-O3" CXXFLAGS="-O3" ./bootstrap --abi 64 --make-jobs $PJOBS --prefer-pkgsrc yes --prefix $PREFIX --unprivileged --sysconfdir $CONFDIR --varbase $VARBASE || exit 1
...
if [ "$solaris" = true ]; then
  ...
  props="CXXFLAGS-=\t\t-O3 MAKE_JOBS=\t\t$PJOBS SKIP_LICENSE_CHECK=\tyes"
fi
