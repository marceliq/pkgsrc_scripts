...
PKGSRC_BASE=/app
PREFIX=/app/common/postgresql11

CVS_BRANCH="HEAD"
#PKGSRC_URL="https://cdn.netbsd.org/pub/pkgsrc/current/pkgsrc.tar.gz"

PKGSRC_MODULES="databases/postgresql11-server"
...
VERSION=`$PREFIX/sbin/pkg_info | ${GREP} postgresql | ${AWK} -F '-' '{print $2}' | ${AWK} '{print $1}'`
...
# vytvoreni balicku
(cd $PREFIX-${VERSION}/.. && tar czf postgresql-${VERSION}-`uname -s | tr '[:upper:]' '[:lower:]'`-`uname -p`.tar.gz redis-${VERSION}) || exit 1
