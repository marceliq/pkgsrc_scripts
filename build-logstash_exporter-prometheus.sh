...
PKGSRC_BASE=/app
PREFIX=/app/elastic/dists/prometheus/logstash_exporter
PKGSRC_URL="https://cdn.netbsd.org/pub/pkgsrc/current/pkgsrc.tar.gz"

PKGSRC_MODULES="rb/prometheus-logstash_exporter"
...
VERSION=`$PREFIX/sbin/pkg_info | ${GREP} prometheus-logstash-exporter | ${AWK} -F '-' '{print $4}' | ${AWK} '{print $1}'`
...
# vytvoreni balicku
(cd $PREFIX/.. && tar czf logstash_exporter-prometheus-${VERSION}-`uname -s | tr '[:upper:]' '[:lower:]'`-`uname -p`.tar.gz logstash_exporter/bin/prometheus-logstash-exporter) || exit 1
