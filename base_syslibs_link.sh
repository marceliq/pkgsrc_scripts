...
if [ "$solaris" = true ]; then
  ...
  if [ ! -d "${PKGSRC_BASE}/pkgsrc/bootstrap/work" ]; then
    LDFLAGS="-Wl,--strip-all" SHARED_LDFLAGS="-Wl,--strip-all" ./bootstrap --abi 64 --make-jobs $PJOBS --prefer-native yes --prefix $PREFIX --unprivileged --sysconfdir $CONFDIR --varbase $VARBASE || exit 1
