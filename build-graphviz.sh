...
PKGSRC_BASE=/app
PREFIX=/app/common/graphviz
#CVS_BRANCH="pkgsrc-2018Q4"
PKGSRC_URL="https://cdn.netbsd.org/pub/pkgsrc/current/pkgsrc.tar.gz"
PJOBS=`nproc`

PKGSRC_MODULES="graphics/graphviz"
...
#props="CFLAGS-=\t\t-O2 CXXFLAGS-=\t\t-O2 CPPFLAGS-=\t\t-O2 MAKE_JOBS=\t\t$PJOBS PKG_OPTIONS.graphviz=\t-x11 PKG_OPTIONS.graphviz+=\t-gd PKG_OPTIONS.graphviz+=\t-gtk PKG_OPTIONS.graphviz+=\t-lua PKG_OPTIONS.graphviz+=\t-perl PKG_OPTIONS.graphviz+=\t-tcl SKIP_LICENSE_CHECK=\tyes"

props="MAKE_JOBS=\t\t$PJOBS PKG_OPTIONS.graphviz=\t-x11 PKG_OPTIONS.graphviz+=\t-gd PKG_OPTIONS.graphviz+=\t-gtk PKG_OPTIONS.graphviz+=\t-lua PKG_OPTIONS.graphviz+=\t-perl PKG_OPTIONS.graphviz+=\t-tcl SKIP_LICENSE_CHECK=\tyes"
...
VERSION=`$PREFIX/sbin/pkg_info | ${GREP} graphviz | ${AWK} -F '-' '{print $2}' | ${AWK} '{print $1}'`
...
# vytvoreni balicku
(cd $PREFIX-${VERSION}/.. && tar czf graphviz-${VERSION}-`uname -s | tr '[:upper:]' '[:lower:]'`-`uname -p`.tar.gz graphviz-${VERSION}) || exit 1
