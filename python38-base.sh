...
props="PYTHON_DEFAULT_VERSION=\t38 PYTHON_VERSION_DEFAULT=\t38 PYPACKAGE=\t\tpython38 PKG_OPTIONS.python38=\t-x11"
...
# python
_nol=`$PREFIX/sbin/pkg_info |$GREP python38 |wc -l`
if [ $_nol -eq 0 ]; then
  (cd ${PKGSRC_BASE}/pkgsrc/lang/python38 && bmake install clean clean-depends) || exit 1
fi
