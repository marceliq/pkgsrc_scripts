...
PKGSRC_BASE=/app
PREFIX=/app/elastic/dists/tools
...
PKGSRC_MODULES="security/mozilla-rootcerts devel/py-pip devel/py-readline devel/py-curses /devel/py-cursespanel textproc/py-yaml devel/py-requests devel/git-base"
...
# vytvoreni balicku
(cd $PREFIX/.. && ${TAR} czf tools-`date +%Y%m%d`-`uname -s | tr '[:upper:]' '[:lower:]'`-`uname -p`.tar.gz tools) || exit 1
