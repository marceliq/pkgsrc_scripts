...
props="PYTHON_DEFAULT_VERSION=\t39 PYTHON_VERSION_DEFAULT=\t39 PYPACKAGE=\t\tpython39 PKG_OPTIONS.python39=\t-x11"
...
# python
_nol=`$PREFIX/sbin/pkg_info |$GREP python39 |wc -l`
if [ $_nol -eq 0 ]; then
  (cd ${PKGSRC_BASE}/pkgsrc/lang/python39 && bmake install clean clean-depends) || exit 1
fi
